...
import {
  ArrowRight, MapPin, Shield, Sparkles, Users, BarChart3, Eye, Zap, Building2,
  CheckCircle2, Smartphone, Activity, Brain, TrendingUp, Calendar,
} from "lucide-react";
...
const MiniKpi = ({ label, value, color, bg }: { label: string; value: number; color: string; bg: string }) => (
...
const Pillar = ({ icon: Icon, title, desc }: { icon: typeof Users; title: string; desc: string }) => (
...
const SolutionCard = ({
  eyebrow, icon: Icon, title, items, cta, variant,
}: {
  eyebrow: string; icon: typeof Users; title: string; items: string[];
  cta: { label: string; to: string }; variant: "primary" | "accent";
}) => (
  <div className={`relative rounded-3xl p-8 overflow-hidden border ${variant === "primary" ? "bg-gradient-primary text-primary-foreground border-transparent shadow-elegant" : "glass-card border-border"}`}>
    <div className="absolute inset-0 bg-grid-pattern opacity-5" />
    <div className="relative">
      <p className={`text-xs font-bold uppercase tracking-widest ${variant === "primary" ? "text-accent-glow" : "text-accent"}`}>{eyebrow}</p>
      <div className="flex items-center gap-3 mt-3">
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${variant === "primary" ? "bg-primary-foreground/15" : "bg-gradient-accent shadow-glow"}`}>
          <Icon className={`w-6 h-6 ${variant === "primary" ? "text-primary-foreground" : "text-accent-foreground"}`} />
        </div>
        <h4 className={`font-display text-2xl font-bold ${variant === "primary" ? "text-primary-foreground" : "text-foreground"}`}>{title}</h4>
      </div>
      <ul className="mt-6 space-y-2.5">
        {items.map((it) => (
          <li key={it} className={`flex items-start gap-2.5 text-sm ${variant === "primary" ? "text-primary-foreground/85" : "text-muted-foreground"}`}>
      ...
      <Button
        asChild
        className={`mt-7 font-bold ${variant === "primary" ? "bg-accent text-accent-foreground hover:opacity-95" : "bg-primary text-primary-foreground hover:bg-primary/90"}`}
      >
        <Link to={cta.to}>{cta.label} <ArrowRight className="ml-1.5 w-4 h-4" /></Link>
      </Button>
    </div>
  </div>
);

const LiveStat = ({ icon: Icon, label, value, accent }: { icon: typeof Users; label: string; value: string | number; accent?: boolean }) => (
  <div className={`rounded-2xl p-6 border animate-fade-in-up ${accent ? "bg-gradient-primary text-primary-foreground border-transparent shadow-elegant" : "glass-card border-border"}`}>
    <Icon className={`w-5 h-5 ${accent ? "text-accent-glow" : "text-accent"}`} />
    <p className={`font-display text-3xl font-bold mt-3 ${accent ? "text-primary-foreground" : "text-foreground"}`}>{value}</p>
    <p className={`text-xs font-semibold mt-1 ${accent ? "text-primary-foreground/75" : "text-muted-foreground"}`}>{label}</p>
  </div>
);
